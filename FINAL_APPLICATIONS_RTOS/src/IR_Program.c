...
void HIR_VoidInit(void)
{
	...
	MGPIO_SetInputPinType(IR_RIGHT_PORT,IR_RIGHT_PIN,PULL_DOWN);
	/* to init IR_Left   */
	MGPIO_SetMode(IR_LEFT_PORT ,IR_LEFT_PIN,INPUT);
	MGPIO_SetInputPinType(IR_LEFT_PORT ,IR_LEFT_PIN,PULL_DOWN);
/*to init IR_center*/
	MGPIO_SetMode(IR_CENTER_PORT ,IR_CENTER_PIN,INPUT);
	MGPIO_SetInputPinType(IR_CENTER_PORT ,IR_CENTER_PIN,PULL_DOWN);
...
uint8 HIR_CenterState(void)
{
	//to read value of IR_Center
	    uint8 Local_uint8Centerstate;
		MGPIO_readPin(IR_CENTER_PORT ,IR_CENTER_PIN,&Local_uint8Centerstate);

		return  Local_uint8Centerstate;
